...
from echoline.dependencies import ApiKeyDependency, get_config, get_executor_registry
from echoline.logger import setup_logger
from echoline.routers.chat import (
    router as chat_router,
)
from echoline.routers.diarization import (
    router as diarization_router,
)
from echoline.routers.misc import (
    public_router as misc_public_router,
)
from echoline.routers.misc import (
    router as misc_router,
)
from echoline.routers.models import (
    router as models_router,
)
from echoline.routers.realtime_rtc import (
    router as realtime_rtc_router,
)
from echoline.routers.realtime_ws import (
    router as realtime_ws_router,
)
from echoline.routers.speech import (
    router as speech_router,
)
from echoline.routers.speech_embedding import (
    router as speech_embedding_router,
)
from echoline.routers.stt import (
    router as stt_router,
)
from echoline.routers.vad import (
    router as vad_router,
)
from echoline.routers.vad_stream_ws import (
    router as vad_stream_ws_router,
)
from echoline.utils import APIProxyError
...
def create_app() -> FastAPI:
    ...
    if config.otel_exporter_otlp_endpoint:
        from echoline.tracing import setup_telemetry
    ...
    async def _api_proxy_error_handler(_request: Request, exc: APIProxyError) -> JSONResponse:
        ...
        log_level = os.getenv("ECHOLINE_LOG_LEVEL", "INFO").upper()
    ...
    if config.enable_ui:
        ...
        from echoline.ui.app import create_gradio_demo

        app = gr.mount_gradio_app(app, create_gradio_demo(config), path="")

        logger = logging.getLogger("echoline.main")
        if config.host and config.port:
            ...
            logger.info(f"\n\nTo view the gradio web ui of echoline open your browser and visit:\n\n{url}\n\n")
