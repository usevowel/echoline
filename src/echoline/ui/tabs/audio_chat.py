...
from echoline.audio import convert_audio_format
from echoline.config import Config
from echoline.ui.utils import openai_client_from_gradio_req
...
def create_audio_chat_tab(config: Config, api_key_input: gr.Textbox) -> None:
    ...
    async def update_chat_model_dropdown() -> gr.Dropdown:
        # NOTE: not using `openai_client_from_gradio_req` because we aren't intrested in making API calls to `echoline` but rather to whatever the user specified as LLM api
